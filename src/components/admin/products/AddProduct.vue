  ...
  <div class="full-w">
    ...
    <v-row v-else justify="center" align="start">
      ...
      <v-col cols="10" md="4">
        ...
        >
          <h4 class="mb-2">افزودن محصول جدید</h4>
          <!-- title -->
          <v-text-field
            class="full-w rounded-lg"
            color="secondary"
...
export default Vue.extend({
  ...
  methods: {
    submitProduct() {
      store.dispatch("admin/addProduct", {
        image: this.chosenFile,
        images: this.chosenFiles,
      });
    },
  ...
  mounted() {
    ...
    if (id) {
      store.dispatch("admin/updateProductCategory", id);
    }
  },
  validations: {
    name: {
